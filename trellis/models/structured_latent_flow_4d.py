...
from ..modules.transformer import AbsolutePositionEmbedder, FeedForwardNet
...
class SLatFlow4DModel(nn.Module):
    def __init__(
        ...
        pe_mode: Literal["ape", "rope", "rope_t", "rope_4d"] = "ape",
        ...
        staticondition: bool = False,
        clip_cond_channels: int = 1024,
    ):
        super().__init__()
        ...
        self.staticondition = staticondition
        ...
        if pe_mode == "ape" or pe_mode == "rope_t" or pe_mode == "rope_4d":
        ...
        if self.staticondition:
            self.extra_layer = sp.SparseLinear(in_channels + 1, model_channels if io_block_channels is None else io_block_channels[0])
            self.clip_mlp = FeedForwardNet(
                    clip_cond_channels,
                    mlp_ratio=mlp_ratio,
                )
        ...
        self.blocks = nn.ModuleList([
            ModulatedSparseTransformerCross4DBlock(
                ...
                use_rope_4d=(pe_mode == "rope_4d"),
                rope_base=rope_base,
                share_mod=self.share_mod,
                qk_rms_norm=self.qk_rms_norm,
                qk_rms_norm_cross=self.qk_rms_norm_cross,
                expand_self_attn=expand_self_attn,
                staticondition = self.staticondition,
                clip_cond_channels = clip_cond_channels,
            )
    ...
    def initialize_weights(self) -> None:
        ...
        if self.staticondition:
            for block in self.blocks:
                if hasattr(block, 'static_cross_attn') and block.static_cross_attn is not None:
                    attn_module = block.static_cross_attn
                if hasattr(attn_module, 'to_out') and isinstance(attn_module.to_out, nn.Linear):
                    nn.init.constant_(attn_module.to_out.weight, 0)
                    if attn_module.to_out.bias is not None:
                        nn.init.constant_(attn_module.to_out.bias, 0)

        # Zero-out output layers:
        if self.staticondition:
            nn.init.constant_(self.extra_layer.weight, 0)
            nn.init.constant_(self.extra_layer.bias, 0)
        nn.init.constant_(self.out_layer.weight, 0)
        nn.init.constant_(self.out_layer.bias, 0)

    def forward(self, x: sp.SparseTensor, t: torch.Tensor, cond: torch.Tensor, num_frames: int, static=None, mask=None, clip_cond=None, indices=None) -> sp.SparseTensor:
        if self.staticondition:
            s_m = x.replace(torch.cat([static.feats, mask.feats], dim=-1))
            h = self.input_layer(x).type(self.dtype) + self.extra_layer(s_m).type(self.dtype)
        else:
        ...
        if self.staticondition:
            clip_cond = self.clip_mlp(clip_cond)
            clip_cond = clip_cond.type(self.dtype)
        ...
        if self.pe_mode == "ape" or self.pe_mode == "rope_t" or self.pe_mode == "rope_4d":
        ...
        if self.staticondition:
            clip_cond_t = clip_cond.view(clip_cond.shape[0]//2, -1, clip_cond.shape[-1])
            for block in self.blocks:
                h = block(h, t_emb_t, cond_t, num_frames=num_frames_t, clip_cond=clip_cond_t)
        else:
            for block in self.blocks:
