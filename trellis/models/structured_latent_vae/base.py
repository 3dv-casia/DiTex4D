...
from ...modules.sparse.transformer import SparseTransformerBlock, SparseTransformer4DBlock
...
class SparseTransformerBase(nn.Module):
    ...
    def forward(self, x: sp.SparseTensor, num_frames: int) -> sp.SparseTensor:
        h = self.input_layer(x)
        if self.pe_mode == "ape" or self.pe_mode == "rope_t":
            h = h + self.pos_embedder(x.coords[:, 1:])
        h = h.type(self.dtype)
        for block in self.blocks:
            h = block(h, num_frames)
        return h
