...
# -----------------------------------render_frames_time----------------------------------------
def render_frames_time(sample, extrinsics, intrinsics, options={}, colors_overwrite=None, verbose=True, **kwargs):
    renderer = get_renderer(sample[0], **options)
    rets = {}
    frame_number = 16

    for j, (extr, intr) in tqdm(enumerate(zip(extrinsics, intrinsics)), desc='Rendering', disable=not verbose):
        for frame in range(frame_number):
            current_sample = sample[frame]

            if isinstance(current_sample, MeshExtractResult):
                res = renderer.render(current_sample, extr, intr)
                if 'normal' not in rets: rets['normal'] = []
                rets['normal'].append(np.clip(res['normal'].detach().cpu().numpy().transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8))
            else:
                res = renderer.render(current_sample, extr, intr, colors_overwrite=colors_overwrite)
                if 'color' not in rets: rets['color'] = []
                if 'depth' not in rets: rets['depth'] = []
                rets['color'].append(np.clip(res['color'].detach().cpu().numpy().transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8))
                if 'percent_depth' in res:
...
# -----------------------------------render_video_time----------------------------------------
def render_video_time(sample, resolution=512, bg_color=(0, 0, 0), num_frames=300, r=2, fov=40, **kwargs):
    yaws = torch.linspace(0, 2 * 3.1415, num_frames)
    pitch = 0.25 + 0.5 * torch.sin(torch.linspace(0, 2 * 3.1415, num_frames))
    yaws = yaws.tolist()
    pitch = pitch.tolist()
    extrinsics, intrinsics = yaw_pitch_r_fov_to_extrinsics_intrinsics(yaws, pitch, r, fov)
    return render_frames_time(sample, extrinsics, intrinsics, {'resolution': resolution, 'bg_color': bg_color}, **kwargs)
# --------------------------------------------------------------------------------------------
