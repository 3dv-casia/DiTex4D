...
def save_images_as_video(images: Union[torch.Tensor, np.ndarray], path):
    """
    Save a list of images as video [B, C, H, W]
    """
    if isinstance(images, torch.Tensor):
        # Convert to NumPy and reshape from [B, C, H, W] -> [B, H, W, C]
        images = images.detach().cpu().numpy().transpose(0, 2, 3, 1)
    
    elif isinstance(images, np.ndarray):
        if images.ndim != 4 or images.shape[-1] not in [1, 3, 4]:
            raise ValueError("NumPy input must be of shape [B, H, W, C] with C=1,3,4")
    else:
        raise TypeError("images must be a torch.Tensor or np.ndarray")

    if images.dtype == np.float32 or images.dtype == np.float64:
        images = np.clip(images * 255, 0, 255).astype(np.uint8)
    if images.shape[1] == 518:
        images = [cv2.resize(image, (512,512), cv2.INTER_AREA) for image in images]
    imageio.mimsave(path, images, fps=6)


def save_images_with_notes_as_video(images: torch.Tensor, path, notes=None):
    """
    Save a list of images as video with notes.
    """
    ...
    if images.shape[1] == 518:
        images = [cv2.resize(image, (512,512), cv2.INTER_AREA) for image in images]
    images = [notes_on_image(img, notes) for img in images]
