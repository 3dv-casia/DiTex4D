...
def rope_apply(x, grid_sizes, freqs):
    B, T, L, n, head_dim = x.shape
    c = head_dim // 2
    x = x.reshape(B, T * L, n, head_dim)

    # split freqs
    freqs = freqs.split([c - 3*6, 6, 6, 6], dim=1)
    freqs = [f.to(x.device) for f in freqs]

    # loop over samples
    output = []
    for i, (f, d, h, w) in enumerate(grid_sizes.tolist()):
        seq_len = f * d * h * w
        ...
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float64).reshape(
            seq_len, n, -1, 2))
        freqs_i = torch.cat([
            freqs[0][:f].view(f, 1, 1, 1, -1).expand(f, d, h, w, -1),
            freqs[1][:d].view(1, d, 1, 1, -1).expand(f, d, h, w, -1),
            freqs[2][:h].view(1, 1, h, 1, -1).expand(f, d, h, w, -1),
            freqs[3][:w].view(1, 1, 1, w, -1).expand(f, d, h, w, -1)
        ],
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        if use_rope_4d:
            self.freqs = torch.cat([
                rope_params(1024, self.head_dim - 3*12, rope_base),
    ...
    def forward(self, x: torch.Tensor, context: Optional[torch.Tensor] = None, indices: Optional[torch.Tensor] = None, num_frames=None) -> torch.Tensor:
        ...
        if self._type == "self":
            ...
            if self.use_rope_4d:
                q, k, v = qkv.unbind(dim=3) # [b, T, L, num_heads, -1]
                indices = torch.tensor([[num_frames, 16, 16, 16]] * (B // num_frames), dtype=torch.long, device=x.device)
                q = rope_apply(q, indices, self.freqs).to(x.dtype)
                k = rope_apply(k, indices, self.freqs).to(x.dtype)
                qkv = torch.stack([q, k, v], dim=3)
            if self.attn_mode == "full":
                qkv = qkv.reshape(B//num_frames, num_frames*L, 3, self.num_heads, -1)
                if self.qk_rms_norm:
